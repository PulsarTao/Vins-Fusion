...
相机静置桌面时位置发散到**几千米**。逐个排除后定位到四个独立问题，
**必须全部修好**，只修其中一部分仍然会炸。

> 排查这类问题的一个教训：**短时数据不能下结论**。
> 坑一~坑三修好后跑 3 分钟只漂 6.5 cm，我据此宣布修好了；
> 但同一个实例继续跑到 47 分钟就漂到 4215 米 —— 那是坑四。
> 静止发散是个慢过程，验证窗口至少要一小时量级。
...
### 坑四：静止久了加速度计零偏会游走（需要零速修正）

修好前三个坑之后，前 47 分钟表现良好 —— 漂移 0 → 1.53 m，即约
**32.5 毫米/分钟**，这是静止无回环的正常 VIO 漂移。但之后突然崩到公里量级。

追踪 `Ba`（加速度计零偏）的演化就看明白了：

```
帧 1      Ba: 0.002  0.002  0.128
帧 8001   Ba: -0.110  0.491  0.140
帧 16001  Ba:  0.148  0.716  0.147
帧 28001  Ba:  0.219  0.683  0.149
帧 28506  Ba 冻结在 (0.218, 0.684, 0.149)，solver 耗时从 31ms 跌到 2ms
之后      位置按 0.5*Ba*t² 爆炸
```

`Ba_y` 一路游走到 **0.68 m/s²**。而静止时实测加速度是 `(0.234, 0.098, 9.884)`，
真实零偏应该很小 —— 0.68 在物理上不合理。

原因：**加速度计零偏与重力方向在无运动时高度耦合、几乎不可观**，
要靠姿态变化才能解耦。VINS-Fusion 没有零速修正，零偏就一路随机游走，
走到病态区域后优化退化、状态冻结，位置随即二次爆炸。

**解法**：`patches/0003-zero-velocity-update.patch`。
检测到静止时给窗口内每一帧的速度加「速度应为零」的残差。
速度被钉住后，IMU 预积分的速度残差 `r_v = delta_v(Ba,Bg) - R_i^T·g·dt`
就唯一确定了 `Ba` —— 零偏从「不可观、随便游走」变成「有唯一解」。
...
> **已知局限**：严格匀速直线运动时加速度与角速度都无波动，
> 静止检测会误判并错误地把速度钉到零。实际使用中平台总有抖动，
> 阈值又取到实测静止噪声的 10 倍，风险很低；但如果将来出现
> 「缓慢平稳移动时轨迹卡住不动」的现象，先怀疑这里。
...
| 指标 | 修复前 | 修复后 |
|---|---|---|
| 静止位置漂移 | 数分钟内 244 m ~ 7545 m | 见下表 |
| `wait for imu` 死锁 | 47478 次 | **0** |
| `numerical unstable` | 6 ~ 13 次 | **0** |
| 参与优化的特征数 | **0** | 110 ~ 154 |
| `Ba` 加速度计零偏 | 游走到 0.68 m/s² 后冻结 | **稳定在 0.005 ~ 0.02** |
| 图像帧率 | — | 20.0 Hz（宿主机与容器一致） |
| IMU 频率 | — | 200 Hz，零重复时间戳 |
| solver 耗时 | — | 3.5 ~ 15 ms（10 Hz 下余量充足） |

静止漂移分阶段看（**这一栏必须长跑才有意义**）：

| 运行时长 | 修坑一~三 | 再加 ZUPT（坑四） |
|---|---|---|
| 3 分钟 | 6.5 cm | 1.9 cm |
| 47 分钟 | 1.53 m | 见仓库最新实测 |
| 51 分钟 | **4215 m（崩溃）** | — |

陀螺零偏 `Bg = (0.0190, 0.0116, -0.0173)` 与静止时实测的陀螺读数均值
高度吻合 —— 这是估计器工作正常的有力旁证。

> **尚未验证**：以上全部是相机静置桌面的数据。**运动状态下的精度没有测过**。
